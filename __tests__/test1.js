  ...
  () => {
    ...
    it('Testing analytics click event without error', async () => {
      const data = await page.evaluate(async () => {
        const $container = jQuery('#slick-slide30');
        const elementData = $container.find('.hero-slide').data();
        const analyticsData = {
          ...
          promo_click_position: 'component|' + ($container.data().slickIndex + 1),
        ...
        await $container.find('.hero-slide').trigger('click');
